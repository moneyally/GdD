...
└── LivingWorld.Unity/         UnityEngine 의존. 표현 층
```
...
테스트 24개 통과. 그중 3개가 **골든 대조**다 — TypeScript 구현과 C# 구현이
같은 시드에서 같은 결과를 내는지 확인한다:
...
- **Snapshot 저장/복원**: TS 쪽에는 있고 C#에는 아직 없다.
  `World.Restore`는 있으므로 직렬화만 붙이면 된다
